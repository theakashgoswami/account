...
import { User, Camera, Save, Loader2, AlertCircle, CheckCircle2 } from 'lucide-react';
...
export const Profile: React.FC = () => {
  ...
  useEffect(() => {
    if (user) setForm({ name: user.name || '', email: user.email || '', phone: user.phone || '', address: user.address || '', profile_image: user.profile_image || '' });
  }, [user]);
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    setSaving(true);
    setMsg(null);
    const res = await API.updateProfile({ name: form.name, address: form.address, profile_image: form.profile_image });
    if (res.success) {
  ...
  return (
    <div className="container mx-auto max-w-4xl px-4 py-12">
      <header className="mb-10">
        ...
        <p className="mt-2 text-zinc-500">Manage your personal information.</p>
      </header>
